$(document).ready(function(){
    var $price = $(".pd_price span").tetx();    //현재 가격 정보를 저장
    console.log("현재 가격의 원본 정보 : "+$price);
    console.log("현재 가격의 원본 정보의 데이터 타입 :"+ typeof $price);
    var $origin_price = $price.replace(",") //쉼표 제거한 원본 재
    console.log("숫자정보의 데이터 타입 :" + typeof $price);
    ...
    function calc_price(){
       ...
       var opt_default = $(".pd_option select option:selected").text();
       $("")                                                                                                                       (".pd_option select").val();
    //    console.log($opt_val);
     //   console.log(typeof $opt_val);


        $total_price_opt = $total_price * num + padrseFloat($opt_val);
        var $total_price_result = $total_price_opt.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        console.log($total_price_result);
    }

        $("pd_count_box a:first").click(function(){
        if($num < 2){   //현재 구매 수량이 1일 경우 작동 금지
        }else{  //현재 구매 수량이 2 이상일 경우 1씩 감소
            $num--;    //10 -> 9 -> 8... -> 1
            calc_price();
        }
        return false;
...
$(".dark, .close_btn, .cart_btn li:last-child input").click(function(){
    ...
    //#5. \d{3} : 좌측 자리부터 세 자리마다 패턴 구성

    //#6. ?!\d : 숫자 데이터만을 지정 (!). 숫자를 세는 과정에서 역순으로 개수를 세겠다는 의미. 부정형 전방 탐색 (역방향으로 셈)
